...
function renderPlayers(scene) {
    ...
    if (serverUpdate.me.captured) {
        isCaptured = true;
        let human = curPlayers[serverUpdate.humanId];
        player.sprite.setPosition(human.x, human.y - 20);
    }
...
function update() {
    ...
    if (!isCaptured) {
        ...
        updatePosition({
            x: player.sprite.x, y: player.sprite.y, right: player.sprite.flipX, moving: false
        });
    }
}